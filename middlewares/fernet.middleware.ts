...
import { isUserPayload, Application, Bearer, Context, User } from "types";
import { logger } from "utils";
import { httpErrors } from "oak";
import { configs } from "config";

const log = logger({ name: "Fernet Middleware", logLevel: configs.general.env === "development" ? 0 : 1 });
...
async function validSession(sessionId: string, userId: bigint): Promise<boolean> {
  ...
  const valid = await db
    ...
    .then((res: { count: any }[]) => Boolean(res[0]?.count));
