...
const rateLimits = new Map<string, { requests: number; startedAt: number; reset?: string }>();
...
export function ratelimit(timeout: number, maxRequests: number, options?: { bucket?: string }) {
  return async function (context: Context, next: () => Promise<unknown>) {
    ...
    context.response.headers.set(
      "X-Rate-Limit-Reset",
      Math.round(((rateLimit?.startedAt ?? now) + timeout * 1000 - now) / 1000).toString(),
    );
    context.response.headers.set("X-Rate-Limit-Reset-TimeStamp", rateLimit?.reset ?? (now + timeout * 1000).toString());
    console.log(now + Math.round((now + timeout * 1000) / 1000).toString());
    ...
    if (rateLimit) {
      ...
      rateLimits.set(id, {
        ...
        requests: 2,
        startedAt: now,
        reset: (now + Math.round(timeout)).toString(),
      });
    }
